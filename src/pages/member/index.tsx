...
const TableList: React.FC = () => {
  ...
  const columns: ProColumns<TableListItem>[] = [
    ...
    {
      title: '账户',
      dataIndex: 'bitCoinAccounts',
      width:200,
      renderText:(text,record)=>(
        <>
          {
            text.map((item: BitCoinAccount)=>{
              if(item.name==='CNY'){
                return (
                  <div>
                    <span>{item.name}：{item.money+item.frozenMoney}</span>
                    <span style={{marginLeft:8}}><a onClick={()=>change(item,record)}><HighlightOutlined /></a></span>
                  </div>
                );
              }
              return <div>{item.name}：{item.money+item.frozenMoney}</div>;
            })
          }
        </>
      )
